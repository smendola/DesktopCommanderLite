...
[![npm downloads](https://img.shields.io/npm/dw/@smendola/desktop-commander-lite)](https://www.npmjs.com/package/@smendola/desktop-commander-lite)
...
[![smithery badge](https://smithery.ai/badge/@smendola/desktop-commander-lite)](https://smithery.ai/server/@smendola/desktop-commander-lite)
...
> What I'm working on: 
> the full upstream Desktop Commander server consumes 13K of context;
> that's like nothing in Claude, but if you're trying to use it in local LLM
> on a tiny rig, 13K is a huge hit to take.
> My plan here is to trim this server down to bare essentials.
> Remove some tools, and trim down the descriptions of the remaining tools.
> Some of tools attempt to optimize a specific function, but the function could
> still be achieved, though less optimally, with some shell command invoked with
> the start_process tool.
> Furthermore, some tools have a lot of options and a lot of verbiage around how
> to use them; here the text is being trimmed down to the most essential parts,
> and the hope is that the LLM can "figure it out" well enough even with less
> explanation.
> Again, this is all to the end of cutting down the context size. My goal is to
> reduce it from 13K to 6K.  This will make a big difference running on e.g.
> 16GB of VRAM.
...
```
npx @smendola/desktop-commander-lite@latest setup
```

For debugging mode (allows Node.js inspector connection):
```
npx @smendola/desktop-commander-lite@latest setup --debug
```
...
**🗑️ Uninstall:** Run `npx @smendola/desktop-commander-lite@latest remove`
...
To install Desktop Commander for Claude Desktop via [Smithery](https://smithery.ai/server/@smendola/desktop-commander-lite):

1. **Visit the Smithery page:** https://smithery.ai/server/@smendola/desktop-commander-lite
...
```json
{
  "mcpServers": {
    "desktop-commander": {
      ...
      "args": [
        "-y",
        "@smendola/desktop-commander-lite@latest"
      ]
    }
  }
}
...
**✅ Auto-Updates:** Yes - automatically updates when you restart Claude  
**🔄 Manual Update:** Run the setup command again  
**🗑️ Uninstall:** Run `npx @smendola/desktop-commander-lite@latest remove` or  remove the "desktop-commander" entry from your claude_desktop_config.json file
...
**🗑️ Uninstall:** Run `npx @smendola/desktop-commander-lite@latest remove` or remove the cloned directory and remove MCP server entry from Claude config
...
```bash
npx @smendola/desktop-commander-lite@latest remove
```
...
2. **Edit the config file:**
  ...
  **Example - Remove this section:**
  ```json
  {
      "desktop-commander": {
        "command": "npx",
        "args": ["@smendola/desktop-commander-lite@latest"]
      }
  }
  ```
